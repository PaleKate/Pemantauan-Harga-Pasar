...
(chart=new ApexCharts(document.querySelector("#full-stacked-bar"),options)).render();
	...
	options={
		...
		tooltip:{
			...
			y:{
				formatter:function(e){return Math.abs(e)+"%"}
				}
			},
		xaxis:{
			...
			labels:{
				formatter:function(e){return Math.abs(Math.round(e))+"%"}
				}
			},
		legend:{
			offsetY:7
...
(chart=new ApexCharts(document.querySelector("#pattern-bar"),options)).render();
	var labels=Array.apply(null,{length:39}).map(function(e,t){return t+1}),
	options={
		chart:{
			height:450,
			type:"bar",
...
(chart=new ApexCharts(document.querySelector("#image-fill-bar"),options)).render();
	...
	options={
		...
		dataLabels:{
			...
			formatter:function(e,t){return t.w.globals.labels[t.dataPointIndex]+":  "+e},
			offsetX:0,
			dropShadow:{
				enabled:!1
				}
